...
class Learner(BaseLearner):
    ...
    def _eval_cnn(self, loader):
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            ...
            outputs = torch.zeros((inputs.shape[0], self._total_classes)).to(self._device)
            with torch.no_grad():
                image_features = self._network(inputs, zeroshot=True)
            ...
            for class_idx in range(0, self._total_classes):
                ...
                outputs[:, class_idx] = distance

            # outputs measures distance, largest=False, torch.min

            predicts = torch.topk(
                outputs, k=self.topk, dim=1, largest=False, sorted=True
            )[1]
            ...
            if calc_task_acc:
                ...
                pred_task_ids = (torch.min(outputs, dim=1)[1] - self.init_cls) // self.inc + 1
                task_correct += (pred_task_ids.cpu() == task_ids).sum()

                pred_task_y = torch.min(task_logits, dim=1)[1]
