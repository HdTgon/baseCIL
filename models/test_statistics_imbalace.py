    ...
    def visual_adapter_train(self, train_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for class_id in range(len(self.cls_mean)):
                if self.args["model_name"] == 'alldata_statistics_imbalance':
                    num_sampled = 16 if class_id < self._known_classes else self.args["num_sampled"]
            ...
            for inp, tgt in loader:
                logits = self._network(inp, zeroshot=False, fc_only=True, joint_train=False, train=False, task_id=self._cur_task)
